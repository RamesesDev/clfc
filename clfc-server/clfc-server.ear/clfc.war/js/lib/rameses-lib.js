...
BindingUtils.handlers.span = function( elem, controller, idx ){
	...
	var ctx = $ctx(controller.name);
	var expr;
	if( span.data('expr')!=null ) {
		expr = span.data('expr');
	} else {
		expr = unescape( span.html() );
		span.data('expr', expr);
	}
	span.html( expr.evaluate(ctx) );
	
	//bind span elements
	BindingUtils.bind( null, span );
};
...
function Bookmarker( tgt ) {
	...
	this.loadPage = function( inv, params ) {
		...
		//shot all parameters including the passed by the calling code
		params = $.extend( params, WindowUtil.getAllParameters() );
		content.load(inv.page, params, function() {
			var canProceed = true;
			try {
...
function PopupOpener( id, params ) {
	...
	var defaultOptions = {show: '', hide: 'blind', height: 'auto'};
